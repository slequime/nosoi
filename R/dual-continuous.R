...
dualContinuous <- function(length.sim,
  ...
  for (pres.time in 1:length.sim) {
    ...
    if(is.function(pMove.A)) res.A <- makeMoves(res.A, pres.time, moving.full.A,
    ...
    if(is.function(pMove.B)) res.B <- makeMoves(res.B, pres.time, moving.full.B,
    ...
    df.meetTransmit.A <- meetTransmit(res.A, pres.time, positions = c("current.in.x", "current.in.y", "current.env.value"), nContactParsed.A, pTransParsed.A)
    res.B <- writeInfected(df.meetTransmit.A, res.B, pres.time, ParamHost.B)

    #Transmission from B to A
    df.meetTransmit.B <- meetTransmit(res.B, pres.time, positions = c("current.in.x", "current.in.y", "current.env.value"), nContactParsed.B, pTransParsed.B)
