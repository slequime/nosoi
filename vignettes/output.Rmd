...
Formally, the transmission tree is extracted as a dated phylogenetic tree where:  
  - each branch represents the living span of an host;
...
Such a tree is bniary, and has as many tips as the total number of infected hosts,
and as many nodes as then number of transmission events.
...
As an exemple, we simulate a single introduction of a single-host pathogen
between 3 differents states (named "A", "B", and "C"),
with a constant exit and move probabilityn as well as a number of contacts
dependant of the location and host count in each location:
...
(as a `tidytere::treedata` object) and plotted using
[`ggtree`](10.18129/B9.bioc.ggtree):
...
Each color point represents the state, either at transmission (a node)
...
All the functions mentioned above produce a `tidytere::treedata` object.
It is a phylogenetic tree, with nodes and tips anotated with all the charasteristics
...
This makes it easy to export the generated data to other softwares for downstream
analyses, thanks to the package [treeio](https://guangchuangyu.github.io/software/treeio/).
