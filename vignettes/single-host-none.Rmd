...
```{r setupF}
devtools::load_all("~/Dropbox/C-Leuven Projects/Nosoi/Nosoi-Rpackage/nosoi/")
#timeContact
time_contact_fct = function(x){round(rnorm(x, 3, 1), 0)}
...
SimulationA <- nosoiSim(type="single", structure=FALSE,
                        ...
                        prefix.host="H")
```
During the run, Nosoi will report a progress bar, that represent the ration of "current time point" over the `length.sim` parameter provided. It can stop before 100% is reached if the maximum number of infected hosts is met. Once the simulation is finished, it reports the number of time units for which the simulation has run (13), and the maximum number of infected hosts (125). Note that the simulation has stopped here before reaching `length.sim` as it has crossed the `max.infected` threshold set at 100.
