...
For a constant or time-dependent function, with no individual parameter, `param.nContact` is `NA`.
In the case of an individually parameterized `nContact`, the functions for the parameters have to be wrapped into a list, called `param.nContact`.
The name of each function in the list has to have the same name as the parameter it is sampling for `nContact`.
`timeDep.nContact` allows for `nContact` to be dependant of the "absolute time" of the simulation, to account, for example, for seasonality or other external time related covariates. 
