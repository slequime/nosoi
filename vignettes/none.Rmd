...
In case the function uses individual-based parameters, you have to specify a list of functions (called `param.pExit`, `param.nContact` or `param.pTrans`) ( [see Get started](nosoi.html#parameters) ). If no individual-based parameter is used, then they are set to `NA`.
...
> In any case, time since infection and "absolute" time should ALWAYS be designated by `t` and `prestime` respectively, and be in that order and before of the individual-based parameters.
