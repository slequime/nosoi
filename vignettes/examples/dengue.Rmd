...
Here we use [WorldClim v2](http://worldclim.org/version2) data (average temperature (°C) - tavg 10m). The rasters are loaded, cut to fit the study site, and the temperature value averaged for each countries (i.e. states in our simulation) each month. Country spatial polygons come from [GADM](https://gadm.org/data.html).
...
The simulation has run for `r test.nosoiA$total.time` and has infected `r test.nosoiA$host.info.A$N.infected` humans and `r test.nosoiA$host.info.B$N.infected` mosquitoes. The simulation was stopped because it reached the infected human cap `max.infected.A`.
