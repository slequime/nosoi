...
In this animation, green dots represent the infected hosts, each dot being an individual host (beware, dots can overlap). The transparency for each dot allows to visualize superposition of dots, in a certain measure. Time is in days.
...
As interesting as the visualization might be, we can also explore a bit the epidemic parameters, for example the "real" $R_0$, defined as the average number of cases one case generates, often estimated in epidemiological studies. For this we can use the following code:
...
The full transmission chain is a bit messy, because of the number of infected individuals. We can also sub-sample it randomly. We will select here 250 individuals sampled at the time of their exit (either death or recovery):
