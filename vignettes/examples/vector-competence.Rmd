...
Each year, numerous experimental studies aim to evaluate vector species or local population’s competence for vector-borne pathogens, #GB: this sentence/structure isn't clear to me; perhaps 'competence' should be replaced by a different term or the first part of the sentence rewritten?
which is ultimately intended to understand epidemiology and assess local risk of transmission for potentially introduced vector-borne pathogens.
...
- the daily number of infectious contacts with different potential hosts (`nContact`), which would be the number of times per day a human is bitten by a sensitive mosquito, or the number of times per day a mosquito is biting a different human. #GB: replace 'sensitive' by 'susceptible' (sensitive has a different meaning in general)?
...
`pTrans` will of course vary according the the viral isolate studied. EIP was measured dynamically over time as the proportion of mosquitoes with disseminated viral infection. #GB: 'with' or 'which'?
This experimental measure is then a mean of the whole population and follows a logistic function characterized by three parameters:
#GB: mention again where the values below were obtained from.
...
#GB: it would be good so show here what is being printed to screen when you actually run the simulation.

# Analyzing the results and conclusions

To analyse the results, we classify each simulation into 3 categories based on the number of humans infected: over 100 infected, below 100 infected and no secondary cases (i.e. just one infected). We then plot the results as follows:
...
#GB: I think it would be good/helpful to explicitly mention VC again here, as that's what this page is about, i.e. an additional sentence regarding VC related to what is shown in the figure.
